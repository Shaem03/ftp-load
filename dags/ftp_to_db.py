...
def download_file_from_ftp(remote_path, local_path, prefix, suffix, conn_id, ext, **kwargs):
    ...
    date = str(kwargs['execution_date'].day) + '-' + str(kwargs['execution_date'].month) + '-' + str(
        kwargs['execution_date'].year)
...
def bulk_load_sql(table_name, **kwargs):
    from Connection import Activity
    local_filepath = kwargs['ti'].xcom_pull(task_ids='download_file')
    dburl = "sqlite:///ftp.db"
    engine = create_engine(dburl)
    session = Session(bind=engine)
    session.bulk_insert_mappings(Activity, local_filepath)
    return table_name
